...
export class ControllerManager {
  ...
  private async _doStart(): Promise<void> {
    ...
    try {
      const matterCjsDir = dirname(require.resolve("@matter/nodejs"));
      const sqliteModulePath = join(matterCjsDir, "storage", "sqlite", "index.js");
      // eslint-disable-next-line @typescript-eslint/no-require-imports, @typescript-eslint/no-explicit-any
      const sqliteMod = require(sqliteModulePath) as Record<string, any>;
      const { SqliteStorageDriver, SqliteBlobStorageDriver } = sqliteMod;

      if (!SqliteStorageDriver || !SqliteBlobStorageDriver) {
        throw new Error(
          "SqliteStorageDriver or SqliteBlobStorageDriver not exported from sqlite module — " +
      ...
      logger.warn(
        `SQLite storage driver unavailable — falling back to file storage: ${
